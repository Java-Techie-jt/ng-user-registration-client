...
public class PolicyConnection {
    ...
    private SOAPMessage createSoapRequest() throws SOAPException, ProgramException, DBConnectionError   {
        ...
        //Arunn requested to change the order
        ...
        SOAPElement element62 = element4.addChildElement("DistributionChannel", "ns1");
        if(null != Integer.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getDistributionchannel())){
        	element62.addTextNode(Integer.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getDistributionchannel()));
        } else {
        	element62.addTextNode("");
        }
        ...
        SOAPElement element63 = element4.addChildElement("ExpaceShare","ns1");
        if(this.emailbean.getRiskEnvelopeType().getSubmissionData().getExpaceshare() != null){
        if(null != Float.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getExpaceshare())){
        	element63.addTextNode(Float.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getExpaceshare()));
        } else {
        	element63.addTextNode("0");
        }
        } else {
        	element63.addTextNode("0");
        }
...
//        LOG.info("Branch Code in PolicyConnection:"+this.emailbean.getRiskEnvelopeType().getSubmissionData().getBRANCHCODE());
...
//        }
        ...
        SOAPElement element44 = element4.addChildElement("NorthAmericanIndustrialClassificationSystemDetails", "ns1");
        
        if(this.emailbean.getRiskEnvelopeType().getSubmissionData().getNaiccode() != 0){
        	
        if (null != Integer.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getNaiccode())) {
        	
        	String naic = Integer.toString(this.emailbean.getRiskEnvelopeType().getSubmissionData().getNaiccode());
        	if(null != this.emailbean.getRiskEnvelopeType().getSubmissionData().getNaicextension()){
        		naic = naic + this.emailbean.getRiskEnvelopeType().getSubmissionData().getNaicextension();
        	}
        	element44.addTextNode(naic.trim());
        } else {
        	element44.addTextNode("");
        } 
    ...
    private boolean createSoapResponse(SOAPMessage soapResponse) {
        ...
        if (null != rt && null != rt.getOverAllResult().getValue() && "Success".equalsIgnoreCase(rt.getOverAllResult().getValue())) {
        	...
        	if(emailbean.getRiskEnvelopeType().getRiskIndex().getPOLNUMBER()!=null&& emailbean.getRiskEnvelopeType().getRiskIndex().getPOLNUMBER().length()!=0){
        		...
        		if(length>=12)
        		this.emailbean.setPolicyId(emailbean.getRiskEnvelopeType().getRiskIndex().getPOLNUMBER().substring(0,11));
        		else
        			this.emailbean.setPolicyId(emailbean.getRiskEnvelopeType().getRiskIndex().getPOLNUMBER());
