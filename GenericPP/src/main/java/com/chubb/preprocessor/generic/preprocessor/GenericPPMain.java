...
public class GenericPPMain {

	static final Logger LOG = Logger.getLogger(GenericPPMain.class);
	...
	public static void main(String[] args) throws DBConnectionError {
		...
		try {
			...
			if (!resources.isLoadBalanced()) {
				...
				Thread process = new Thread(new ProcessInitiator());
				...
				LOG.info(
						"------------Generic Preprocessor Execution Process ENDS without High Availability settings.-----------");
			}
			// HighAvailability mode - Sync between the two nodes
				...
				if (resources.getConnectionproperties().getProperty(SERVER_NAME).equalsIgnoreCase("server1")) {
					...
					LOG.info(
							"------------Generic Preprocessor Execution Process ENDS with High Availability settings.-----------");
				}
				}
			...
			LOG.fatal(
					"GenericPPMain :: Main :: Unhandled exception occurred in the program and Exiting execution. Error : "
							+ e.getMessage(),
					e);
		}
	}
...
class ProcessInitiator implements Runnable {
	...
	public void run() {
		...
		try {
			...
			String directory = resources.getConnectionproperties().getProperty(INPUT_SHARED_LOCATION);
			...
			ThreadRunner.initializeThreads(directories, directory);
