package com.chubb.workview.documentloader.dao;
...
//						Encryption.decrypt(props.getProperty("JDBC.Password").trim(), CIPHER));
	...
	public List<String> executeQuery(String queryName, String fileNumber, String ctryname)
			throws SQLException, DBConnectionError {
		LOG.debug("Entered executeQuery method to execute a DB Query");
		ArrayList<String> tmpList = new ArrayList<>();
		int ctry_code = Integer.parseInt(ctryname);
		...
		try {
			...
			pstatement.setString(1, fileNumber);

			pstatement.setInt(2, ctry_code);

			resultSet = pstatement.executeQuery();

			while (resultSet.next()) {
				LOG.info("file number exist " + fileNumber + " for country " + ctryname);
				tmpList.add(
						resultSet.getString(resources.getConstantproperties().getProperty(ROUTING_TABLE_POL_NUMBER)));
	...
	public synchronized boolean insertDocDetais(DocumentBean documentBean) throws DBConnectionError {
		...
		try {
			...
			pstatement.setString(3, documentBean.getParentFileType());
			...
			pstatement.setString(7,
					resources.getConstantproperties().getProperty(documentBean.getCountryCode() + "_team"));
	...
	public Map<String, String> getCtryName() throws SQLException, DBConnectionError {
		...
		try {
			...
			pstatement = this.appDBConnection
					.prepareStatement(resources.getConstantproperties().getProperty("countrycode"));
